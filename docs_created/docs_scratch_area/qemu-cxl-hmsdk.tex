...
Official QEMU CXL doc : \href{https://www.qemu.org/docs/master/system/devices/cxl.html}{Qemu\_website}
...
DAMO Usage : \href{https://github.com/awslabs/damo/blob/next/USAGE.md}{doc}
...
\begin{itemize}
    ...
    \item numa\_balancing enabled but damon stopped
    \item both hmsdk/damo and numa\_balancing enabled
    \item neither hmsdk/damo nor numa\_balancing enabled
\end{itemize}

\textbf{NOTE}: From usage guidelines of hmsdk, it is by default enabled for a cgroup and can also be enabled globally. I tried both, one default with group and adding gupstoy pid to cgroup/hmsdk.procs and other globally with -g flag. Now, main difference I came across in both is that \textit{pgmigrate\_success} and \textit{pgmigrate\_fail} came 0 for cgroup case and non-zero globally. Otherwise, pgpromote, demote and numa\_pages\_migrated were 0 for both. Not sure if this is expected. Please go through them if needed.
...
PS: You can boot the VM yourself. You can find it at my account at : /home/roopam/epfl-research-intern/qemu-vms/epfl-vm.img. The three scripts used are \texttt{cxl-vm.sh}, \texttt{new-hope.sh} and \texttt{boot.sh}. Inside the VM, please select Linux 6.12+ (it's also the default option in the grub menu).
