...
About the \texttt{qemu-system-x86\_64: virtio: bogus descriptor or out of resources} issue, LLM suggested me perhaps it is an I/O issue, and looked like that since \texttt{./memeater --membind 1} had worked (although slowly), so suggested me this script (saved in `experiments/qemu-scripts/cxl-vm-1.sh`) :
...
So this script turned out to be helpful, and all subsequent experiments were done with this (saved in `experiments/qemu-scripts/numa-tiering.sh`):

\begin{lstlisting}[language=bash]
#!/bin/bash

VAR_FAST_MEM_SIZE=20
VAR_SLOW_MEM_SIZE=30
